...
import { initDatabase, prepare, saveDatabase } from './database.js';
...
async function startServer() {
  ...
  app.get('/api/projects', (req, res) => {
    const projects = prepare('SELECT * FROM projects ORDER BY created_at DESC').all();
  ...
  app.post('/api/projects', (req, res) => {
    ...
    const result = prepare('INSERT INTO projects (name, description) VALUES (?, ?)').run(name, description || null);
    const project = prepare('SELECT * FROM projects WHERE id = ?').get(result.lastInsertRowid);
    res.json(project);
  });

  app.delete('/api/projects/:id', (req, res) => {
    prepare('DELETE FROM estimations WHERE project_id = ?').run(req.params.id);
  ...
  app.post('/api/projects/:id/upload', upload.single('file'), async (req, res) => {
    try {
      ...
      prepare(`
        UPDATE projects 
        SET document_filename = ?, parsed_content = ?, updated_at = datetime('now')
        WHERE id = ?
  ...
  app.post('/api/estimate', (req, res) => {
    ...
    const project = prepare('SELECT * FROM projects WHERE id = ?').get(input.project_id);
    if (!project) return res.status(404).json({ error: 'Project not found' });
    ...
    prepare('UPDATE projects SET status = ? WHERE id = ?').run('estimated', input.project_id);

    const estimation = prepare('SELECT * FROM estimations WHERE id = ?').get(result.lastInsertRowid);
    res.json({
      ...estimation,
  ...
  app.get('/api/projects/:id/estimations', (req, res) => {
    const estimations = prepare('SELECT * FROM estimations WHERE project_id = ? ORDER BY created_at DESC').all(req.params.id);
  ...
  app.get('/api/estimations/:id/export/csv', (req, res) => {
    const estimation = prepare('SELECT * FROM estimations WHERE id = ?').get(req.params.id);
    if (!estimation) return res.status(404).json({ error: 'Estimation not found' });

    const project = prepare('SELECT name FROM projects WHERE id = ?').get(estimation.project_id);
  ...
  app.get('/api/task-classifications', (req, res) => {
    const classifications = prepare('SELECT * FROM task_classifications WHERE is_active = 1').all();
  ...
  app.post('/api/task-classifications', (req, res) => {
    ...
    const tc = prepare('SELECT * FROM task_classifications WHERE id = ?').get(result.lastInsertRowid);
    res.json(tc);
  });
  ...
  app.put('/api/task-classifications/:id', (req, res) => {
    ...
    prepare(`
      ...
      SET name = ?, category = ?, base_effort_days = ?, complexity_low_factor = ?, complexity_normal_factor = ?, complexity_high_factor = ?, updated_at = datetime('now')
      WHERE id = ?
    `).run(name, category, base_effort_days, complexity_low_factor, complexity_normal_factor, complexity_high_factor, req.params.id);
    
    const tc = prepare('SELECT * FROM task_classifications WHERE id = ?').get(req.params.id);
    res.json(tc);
  });

  app.delete('/api/task-classifications/:id', (req, res) => {
    prepare('UPDATE task_classifications SET is_active = 0 WHERE id = ?').run(req.params.id);
  ...
  app.get('/api/parameters', (req, res) => {
    const params = prepare('SELECT * FROM estimation_parameters').all();
    res.json(params);
  });
  ...
  app.post('/api/parameters', (req, res) => {
    ...
    try {
      const result = prepare('INSERT INTO estimation_parameters (name, value, description, category) VALUES (?, ?, ?, ?)').run(name, value, description, category);
      const param = prepare('SELECT * FROM estimation_parameters WHERE id = ?').get(result.lastInsertRowid);
      res.json(param);
    } catch (error) {
  ...
  app.put('/api/parameters/:id', (req, res) => {
    ...
    prepare("UPDATE estimation_parameters SET value = ?, description = ?, updated_at = datetime('now') WHERE id = ?").run(value, description, req.params.id);
    const param = prepare('SELECT * FROM estimation_parameters WHERE id = ?').get(req.params.id);
    res.json(param);
  });

  app.delete('/api/parameters/:id', (req, res) => {
    prepare('DELETE FROM estimation_parameters WHERE id = ?').run(req.params.id);
    res.json({ message: 'Deleted' });
  });
